...
func validateOutputDir() {
	if outputDir == "" {
		home := homeDir()
		homeDownloadDir := filepath.Join(home, "vmd-downloads")
...
func validateCredentials(cmd *cobra.Command) {
	user_ok := validateVarEnv(&username, "VMD_USER")
	pass_ok := validateVarEnv(&password, "VMD_PASS")
	if !user_ok || !pass_ok {
		...
		fmt.Fprintf(os.Stderr, "or you must export them as VMD_USER and VMD_PASS environment variables.\n\n")
		cmd.Usage()
		os.Exit(1)
	}
}
